...
public interface ParecerRepository extends JpaRepository<Parecer, Long> {
	@Query("select new com.softplan.fullstack.dto.ParecerDTO(p.id, p.parecer, pp.usuario.nome) from Parecer p left join p.parecerProcesso pp left join pp.processo pr where pr.id = ?1")
